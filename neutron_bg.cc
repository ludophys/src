...
auto my_generator(const my& my) {
  return [&](G4Event* event) {
    ...
    if(src_a <= area_1)
    {
      ...
      z = - src_z/2.;
      if(dir_centered){
	G4double m_dir = std::sqrt(x*x + y*y + z*z);
        x_dir = - x/m_dir;
        y_dir = - y/m_dir;
        z_dir = - z/m_dir;
      }
    ...
    std::uniform_real_distribution<> dist_flux(0.0, 1.0);
    if (!inFile.is_open()) {
    ...
    G4double p_flux = dist_flux(gen) * somme;
    auto it = std::lower_bound(cdf.begin(), cdf.end(), p_flux);
    size_t index = std::distance(cdf.begin(), it);
    G4double c1 = cdf[index-1];
    G4double c2 = cdf[index];
    G4double e1 = energy[index-1];
    G4double e2 = energy[index];
    G4double sampled_energy = e1 + (p_flux - c1) * (e2 - e1) / (c2 - c1);
    //G4double sampled_energy =1;
    ...
    //std::cout << "e="<<sampled_energy*MeV<<" p="<<p_flux <<" c1="<<c1<<" c2="<<c2<<" e1="<<e1 <<" e2="<<e2 << std::endl;
    particle_gun -> SetParticleEnergy(sampled_energy*MeV);
    particle_gun -> SetParticlePosition({x, y, z});
    particle_gun -> SetParticleMomentumDirection({x_dir, y_dir ,z_dir});
    particle_gun -> GeneratePrimaryVertex(event);
    //output << n_event << "   " << sampled_energy << " " << surface << " " << x << "  " << y << "  " << z << "  " << x_dir << "  " << y_dir << "  " << z_dir << std::endl;
  };
}
...
n4::sensitive_detector* sensitive_detector(unsigned& n_event) {
  auto process_hits = [&] (G4Step* step) {
	...
	output<<n_event<< " " << initial_k_energy<< "  " << k_energy_left << std::endl;}
	//output<<n_event<<"  "<<trk_id<<"  "<< parentID<< "  " << name <<"  "<<process<< "  " << cell << "  " << initial_k_energy<< "  " << k_energy_left<<"  "<< secondaries->size()<<std::endl;}
    ...
    if ((tempInitialEnergies.find(identifier) != tempInitialEnergies.end() && track -> GetTrackStatus() == fStopAndKill && name != "neutron" && name != "gamma") || (tempInitialEnergies.find(identifier) != tempInitialEnergies.end() && process == "Transportation" && name != "neutron" && name != "gamma") ) {
        ...
        if (sec_energy != sec_final_energy) {
            output<< n_event << " " << sec_energy << " " << sec_final_energy << std::endl;
...
n4::actions* create_actions(my& my, unsigned& n_event, int* ene_count)
{
  auto my_run_action_end = [&] (const G4Run*) {
    std::cout<<"Neutrons in ch2: "<<my.ch2_counter<<"; in pb: "<<my.lead_counter<<"; in the vessel: "<<my.vessel_counter<<"; in the drift vessel: "<<my.drift_counter<<std::endl;
  };
  auto my_event_action_begin = [&] (const G4Event*) {
    // Reset variables to zero for the new event.
    my.flag_ch2 = false;
    my.flag_lead = false;
    my.flag_vessel = false;
    my.flag_drift = false;
  };
  ...
  auto per_step = [&] (const G4Step* step) {
    ...
    auto parentID = track -> GetParentID();
    auto pre = step -> GetPreStepPoint();
    G4double initial_k_energy = pre -> GetKineticEnergy();
    auto post = step -> GetPostStepPoint();
    G4double k_energy_left = post -> GetKineticEnergy();
    auto process = post -> GetProcessDefinedStep() -> GetProcessName();
    //std::cout<<n_event<<", trackID=" <<trk_id<< " ,parent = "<< parentID<< ", " << name <<", process= "<< process<< " , previous energy: " << initial_k_energy<< ", the post cell:" << current_vol << " , energy:" << k_energy_left<<" , secondaries="<< secondaries->size()<<std::endl;
    if(name == "neutron")
    {
      if(0){}
      #if SHIELD_CH2_ON
      else if(my.flag_ch2 == false && current_vol == "ch2_hollow")
      {
        my.flag_ch2 = true;
        my.ch2_counter++;
      }
      #endif
      #if SHIELD_PB_ON
      else if(my.flag_lead == false && current_vol == "pb_hollow")
      {
        my.flag_lead = true;
        my.lead_counter++;
      }
      #endif
      else if(my.flag_vessel == false && (current_vol == "vessel" || current_vol == "closure_vessel1" || current_vol == "closure_vessel2"))
      {
        my.flag_vessel = true;
        my.vessel_counter++;
      }
      else if(my.flag_drift == false && current_vol == "gasdrift")
      {
        my.flag_drift = true;
        my.drift_counter++;
      }
...
int main(int argc, char* argv[]) {
  ...
  inFile.open("flux/flux_mark_uniform.txt");
  my my;

  G4int physics_verbosity = 0;
  int ene_count[2000] = {0};
  ...
  n4::run_manager::create()
    ...
    .actions(create_actions(my, n_event,ene_count))
